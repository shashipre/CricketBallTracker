...
def process_video_with_hit_detection(video_path, output_path):
    ...
    ball_lost_counter = 0
    min_detection_confidence = 0.7  # Minimum confidence threshold for valid detection
    ...
    result_image = np.ones((500, 1000, 3), dtype=np.uint8) * 255  # White background
    if hit_count > 5:
        text = "HIT!"
    ...
    cv2.putText(result_image, text, (text_x, text_y), cv2.FONT_HERSHEY_SIMPLEX, 5, text_color, 10)
    cv2.imshow("Result", result_image)
    cv2.waitKey(3000)  # Display for 3 seconds
    cv2.destroyAllWindows()

# Process the latest video only
def process_latest_video(video_folder, output_folder):
    # Get list of videos in the folder sorted by modification time (latest first)
    videos = [f for f in os.listdir(video_folder) if f.endswith('.mp4')]
    if not videos:
        print("No videos found in the folder.")
        return
    
    # Get the most recent video based on modification time
    ...
    output_video_path = os.path.join(output_folder, f"processed_{latest_video}")
    
    print(f"Processing latest video: {latest_video}")
    process_video_with_hit_detection(video_path, output_video_path)

if __name__ == "__main__":
    video_folder = r"C:\Users\shash\OneDrive\Documents\Desktop\Sem-3\DP\Cricket PItch Detector\Cricket4"  # Update path to your recorded videos folder
    ...
    process_latest_video(video_folder, output_folder)
